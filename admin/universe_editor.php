...
if ($action == "doexpand")
{
    $result = $db->Execute("SELECT sector_id FROM ".\BlackNova\Services\Db::table('universe')." ORDER BY sector_id ASC");
    ...
    if (!$result->EOF)
    {
        ...
        while (!$result->EOF)
        {
            ...
            $resx = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET distance = ? WHERE sector_id = ?", array($distance, $row['sector_id']));
        ...
        \BlackNova\Services\Db::logDbErrors($db, $trans_status, __LINE__, __FILE__);
    }
}
