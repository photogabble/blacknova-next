...
if (empty ($_POST['user']))
{
    $res = $db->Execute("SELECT ship_id, character_name FROM ".\Bnt\Db::table('ships')." ORDER BY character_name");
...
else
{
    ...
    {
        $res = $db->Execute("SELECT * FROM ".\Bnt\Db::table('ships')." WHERE ship_id=?;", array($_POST['user']));
        Bnt\Db::logDbErrors($db, $res, __LINE__, __FILE__);
        $row = $res->fields;
