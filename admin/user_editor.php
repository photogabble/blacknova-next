...
if (empty ($_POST['user']))
{
    $res = $db->Execute("SELECT ship_id, character_name FROM ".\BlackNova\Services\Db::table('ships')." ORDER BY character_name");
...
else
{
    ...
    {
        $res = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." WHERE ship_id=?;", array($_POST['user']));
        \BlackNova\Services\Db::logDbErrors($db, $res, __LINE__, __FILE__);
        $row = $res->fields;
