...
$result3 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('sector_defence')." WHERE sector_id=? and defence_type ='M'", array($sector));
...
if ($num_defences > 0 && $total_sector_mines > 0 && !$owner && $shipavg > $mine_hullsize)
{
    ...
    if ($mine_owner['team'] != $playerinfo['team'] || $playerinfo['team'] == 0)
    {
        ...
        if ($playerinfo['dev_minedeflector'] >= $roll)
        {
            ...
            $result2 = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET dev_minedeflector = dev_minedeflector - ? WHERE ship_id = ?", array($roll, $playerinfo['ship_id']));
            \BlackNova\Services\Db::logDbErrors($db, $result2, __LINE__, __FILE__);
        }
        else
        {
            ...
            if ($playershields >= $mines_left)
            {
                ...
                $result2 = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_energy = ship_energy - ?, dev_minedeflector = 0 WHERE ship_id = ?", array($mines_left, $playerinfo['ship_id']));
                \BlackNova\Services\Db::logDbErrors($db, $result2, __LINE__, __FILE__);
                if ($playershields == $mines_left)
                {
            ...
            else
            {
                ...
                if ($playerinfo['armor_pts'] >= $mines_left)
                {
                    ...
                    $result2 = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET armor_pts = armor_pts - ?, ship_energy = 0, dev_minedeflector = 0 WHERE ship_id = ?", array($mines_left, $playerinfo['ship_id']));
                    \BlackNova\Services\Db::logDbErrors($db, $result2, __LINE__, __FILE__);
                    if ($playerinfo['armor_pts'] == $mines_left)
                    {
