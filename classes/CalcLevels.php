...
class CalcLevels
{
    ...
    public static function planetBeams($db, $ownerinfo, $base_defense, $planetinfo)
    {
        ...
        $res = $db->Execute("SELECT beams FROM ".\BlackNova\Services\Db::table('ships')." WHERE planet_id = ? AND on_planet = 'Y';", array($planetinfo['planet_id']));
    ...
    public static function planetShields($db, $ownerinfo, $base_defense, $planetinfo)
    {
        ...
        $res = $db->Execute("SELECT shields FROM ".\BlackNova\Services\Db::table('ships')." WHERE planet_id = ? AND on_planet = 'Y';", array($planetinfo['planet_id']));
        Db::logDbErrors($db, $res, __LINE__, __FILE__);
    ...
    public static function planetTorps($db, $ownerinfo, $planetinfo, $base_defense, $level_factor)
    {
        ...
        $res = $db->Execute("SELECT torp_launchers FROM ".\BlackNova\Services\Db::table('ships')." WHERE planet_id = ? AND on_planet = 'Y';", array($planetinfo['planet_id']));
        Db::logDbErrors($db, $res, __LINE__, __FILE__);
        if ($res instanceof ADORecordSet)
        {
            while (!$res->EOF)
