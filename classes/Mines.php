...
class Mines
{
    ...
    {
        $secdef_result = $db->Execute("SELECT * FROM ".\Bnt\Db::table('sector_defence')." WHERE sector_id = ? AND defence_type ='M' ORDER BY QUANTITY ASC", array($sector));
        ...
        if ($secdef_result instanceof ADORecordSet)
        {
            while (!$secdef_result->EOF && $num_mines > 0)
            {
                ...
                {
                    $update_res = $db->Execute("UPDATE ".\Bnt\Db::table('sector_defence')." SET quantity = quantity - ? WHERE defence_id = ?", array($num_mines, $row['defence_id']));
