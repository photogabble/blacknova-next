...
class NewsGateway // Gateway for SQL calls related to Players
{
    ...
    public function selectNewsByDay($day)
    {
        ...
        $sql = "SELECT * FROM ".\BlackNova\Services\Db::table('news')." WHERE date > :start AND date < :end ORDER BY news_id";
        $stmt = $this->pdo_db->prepare($sql);
        $stmt->bindValue(':start', $day . ' 00:00:00');
        $stmt->bindValue(':end', $day . ' 23:59:59');
        $stmt->execute();
        \BlackNova\Services\Db::logDbErrors($this->pdo_db, $sql, __LINE__, __FILE__); // Log errors, if there are any
