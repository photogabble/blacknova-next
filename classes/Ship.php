...
class Ship
{
    ...
    {
        $own_pl_result = $db->Execute("SELECT * FROM ".\Bnt\Db::table('planets')." WHERE owner = ?", array($ship_id));
        ...
        if ($own_pl_result instanceof ADORecordSet)
        {
            while (!$own_pl_result->EOF)
            {
                ...
                $on_pl_result = $db->Execute("SELECT * FROM ".\Bnt\Db::table('ships')." WHERE on_planet = 'Y' AND planet_id = ? AND ship_id <> ?", array($row['planet_id'], $ship_id));
                ...
                if ($on_pl_result instanceof ADORecordSet)
                {
                    while (!$on_pl_result->EOF)
                    {
                        ...
                        $uppl_res = $db->Execute("UPDATE ".\Bnt\Db::table('ships')." SET on_planet = 'N',planet_id = '0' WHERE ship_id = ?", array($cur['ship_id']));
