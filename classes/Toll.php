...
class Toll
{
    ...
    {
        $select_def_res = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('sector_defence')." WHERE sector_id=? AND defence_type ='F'", array($sector));
        ...
        if ($select_def_res instanceof ADORecordSet)
        {
            while (!$select_def_res->EOF)
            {
                ...
                $res = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET credits=credits + ? WHERE ship_id = ?", array($toll_amount, $row['ship_id']));
