...
class Traderoute
{
    public static function traderouteEngage($db, $pdo_db, $lang, $j, $langvars)
    {
        ...
        if ($traderoute['source_type'] == 'P')
        {
            ...
            $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id=?", array($traderoute['source_id']));
        ...
        {
            $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('planets')." WHERE planet_id=? AND (owner = ? OR (corp <> 0 AND corp = ?));", array($traderoute['source_id'], $playerinfo['ship_id'], $playerinfo['team']));
            \BlackNova\Services\Db::logDbErrors($db, $result, __LINE__, __FILE__);
            if (!$result || $result->EOF)
            {
            ...
            $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id=?", array($source['sector_id']));
            \BlackNova\Services\Db::logDbErrors($db, $result, __LINE__, __FILE__);
        ...
        {
            $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id=?", array($traderoute['dest_id']));
            \BlackNova\Services\Db::logDbErrors($db, $result, __LINE__, __FILE__);
        ...
        elseif (($traderoute['dest_type'] == 'L') || ($traderoute['dest_type'] == 'C'))  // Get data from planet table
        {
            ...
            $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('planets')." WHERE planet_id=? AND (owner = ? OR (corp <> 0 AND corp = ?));", array($traderoute['dest_id'], $playerinfo['ship_id'], $playerinfo['team']));
            \BlackNova\Services\Db::logDbErrors($db, $result, __LINE__, __FILE__);
            ...
            $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id=?", array($dest['sector_id']));
            \BlackNova\Services\Db::logDbErrors($db, $result, __LINE__, __FILE__);
            if (!$result || $result->EOF)
            {
        ...
        {
            $query = $db->Execute("SELECT link_id FROM ".\BlackNova\Services\Db::table('links')." WHERE link_start=? AND link_dest=?", array($source['sector_id'], $dest['sector_id']));
            ...
            {
                $query = $db->Execute("SELECT link_id FROM ".\BlackNova\Services\Db::table('links')." WHERE link_start=? AND link_dest=?", array($dest['sector_id'], $source['sector_id']));
                \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
                if ($query->EOF)
                {
        ...
        $result99 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('sector_defence')." WHERE sector_id = ? AND ship_id <> ?", array($source['sector_id'], $playerinfo['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $result99, __LINE__, __FILE__);
        if (!$result99->EOF)
        {
            $fighters_owner = $result99->fields;
            $nsresult = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." WHERE ship_id=?", array($fighters_owner['ship_id']));
            \BlackNova\Services\Db::logDbErrors($db, $nsresult, __LINE__, __FILE__);
        ...
        $result98 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('sector_defence')." WHERE sector_id = ? AND ship_id <> ?", array($dest['sector_id'], $playerinfo['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $result98, __LINE__, __FILE__);
        if (!$result98->EOF)
        {
        ...
        if ($traderoute['source_type'] == 'P')
        {
            $res = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('zones').",".\BlackNova\Services\Db::table('universe')." WHERE ".\BlackNova\Services\Db::table('universe').".sector_id=? AND ".\BlackNova\Services\Db::table('zones').".zone_id=".\BlackNova\Services\Db::table('universe').".zone_id;", array($traderoute['source_id']));
        ...
        if ($traderoute['dest_type'] == 'P')
        {
            $res = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('zones').",".\BlackNova\Services\Db::table('universe')." WHERE ".\BlackNova\Services\Db::table('universe').".sector_id=? AND ".\BlackNova\Services\Db::table('zones').".zone_id=".\BlackNova\Services\Db::table('universe').".zone_id;", array($traderoute['dest_id']));
            \BlackNova\Services\Db::logDbErrors($db, $res, __LINE__, __FILE__);
            $zoneinfo = $res->fields;
            if ($zoneinfo['allow_trade'] == 'N')
            {
        ...
        if ($traderoute['source_type'] == 'P')
        {
            ...
            if ($source['port_type'] == 'special')
            {
                ...
                {
                    $resb = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_colonists=ship_colonists+?, ship_fighters=ship_fighters+?,torps=torps+?, ship_energy=ship_energy+? WHERE ship_id=?", array($colonists_buy, $fighters_buy, $torps_buy, $dist['scooped1'], $playerinfo['ship_id']));
            ...
            else
            {
                ...
                if ($source['port_type'] == 'ore')
                {
                    ...
                    $playerinfo['ship_ore'] += $ore_buy;
                    $sourcecost -= $ore_buy * $ore_price1;
                    $resc = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET port_ore=port_ore-?, port_energy=port_energy-?, port_goods=port_goods-?, port_organics=port_organics-? WHERE sector_id=?", array($ore_buy, $energy_buy, $goods_buy, $organics_buy, $source['sector_id']));
                ...
                if ($source['port_type'] == 'goods')
                {
                    ...
                    $resd = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET port_ore=port_ore-?, port_energy=port_energy-?, port_goods=port_goods-?, port_organics=port_organics-? WHERE sector_id=?", array($ore_buy, $energy_buy, $goods_buy, $organics_buy, $source['sector_id']));
                ...
                if ($source['port_type'] == 'organics')
                {
                    ...
                    $playerinfo['ship_organics'] += $organics_buy;
                    $sourcecost -= $organics_buy * $organics_price1;
                    $rese = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET port_ore=port_ore-?, port_energy=port_energy-?, port_goods=port_goods-?, port_organics=port_organics-? WHERE sector_id=?", array($ore_buy, $energy_buy, $goods_buy, $organics_buy, $source['sector_id']));
                ...
                if ($source['port_type'] == 'energy')
                {
                    ...
                    $playerinfo['ship_energy'] += $energy_buy;
                    $sourcecost -= $energy_buy * $energy_price1;
                    $resf = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET port_ore=port_ore-?, port_energy=port_energy-?, port_goods=port_goods-?, port_organics=port_organics-? WHERE sector_id=?", array($ore_buy, $energy_buy, $goods_buy, $organics_buy, $source['sector_id']));
                ...
                if ($traderoute['circuit'] == '1')
                {
                    $resf = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_ore=?, ship_goods=?, ship_organics=?, ship_energy=? WHERE ship_id=?", array($playerinfo['ship_ore'], $playerinfo['ship_goods'], $playerinfo['ship_organics'], $playerinfo['ship_energy'], $playerinfo['ship_id']));
                    \BlackNova\Services\Db::logDbErrors($db, $resf, __LINE__, __FILE__);
                }
            }
        }
        ...
        elseif (($traderoute['source_type'] == 'L') || ($traderoute['source_type'] == 'C'))
        {
            ...
            if ($traderoute['dest_type'] == 'P')
            {
                ...
                if (($playerinfo['ship_id'] == $source['owner']) || ($playerinfo['team'] == $source['corp']))
                {
                    ...
                    if ($traderoute['circuit'] == '1')
                    {
                        $resg = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_ore=?, ship_goods=?, ship_organics=? WHERE ship_id=?", array($playerinfo['ship_ore'], $playerinfo['ship_goods'], $playerinfo['ship_organics'], $playerinfo['ship_id']));
                ...
                $resh = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('planets')." SET ore=ore-?, goods=goods-?, organics=organics-? WHERE planet_id=?", array($ore_buy, $goods_buy, $organics_buy, $source['planet_id']));
            ...
            elseif (($traderoute['dest_type'] == 'L') || ($traderoute['dest_type'] == 'C'))
            {
                ...
                if ($traderoute['circuit'] == '1')
                {
                    $resi = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET torps=?, ship_fighters=?, ship_colonists=? WHERE ship_id=?", array($playerinfo['torps'], $playerinfo['ship_fighters'], $playerinfo['ship_colonists'], $playerinfo['ship_id']));
                    \BlackNova\Services\Db::logDbErrors($db, $resi, __LINE__, __FILE__);
                }

                $resj = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('planets')." SET colonists=colonists-?, torps=torps-?, fighters=fighters-? WHERE planet_id=?", array($colonists_buy, $torps_buy, $fighters_buy, $source['planet_id']));
        ...
        if ($traderoute['circuit'] == '2')
        {
            ...
            if ($traderoute['dest_type'] == 'P')
            {
                ...
                if ($dest['port_type'] == 'ore')
                {
                    ...
                    $resk = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET port_ore=port_ore-?, port_energy=port_energy-?, port_goods=port_goods-?, port_organics=port_organics-? WHERE sector_id=?", array($ore_buy, $energy_buy, $goods_buy, $organics_buy, $dest['sector_id']));
                ...
                if ($dest['port_type'] == 'goods')
                {
                    ...
                    $resl = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET port_ore=port_ore-?, port_energy=port_energy-?, port_goods=port_goods-?, port_organics=port_organics-? WHERE sector_id=?", array($ore_buy, $energy_buy, $goods_buy, $organics_buy, $dest['sector_id']));
                ...
                if ($dest['port_type'] == 'organics')
                {
                    ...
                    $resm = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET port_ore=port_ore-?, port_energy=port_energy-?, port_goods=port_goods-?, port_organics=port_organics-? WHERE sector_id=?", array($ore_buy, $energy_buy, $goods_buy, $organics_buy, $dest['sector_id']));
                ...
                if ($dest['port_type'] == 'energy')
                {
                    ...
                    $resn = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('universe')." SET port_ore=port_ore-?, port_energy=port_energy-?, port_goods=port_goods-?, port_organics=port_organics-? WHERE sector_id=?", array($ore_buy, $energy_buy, $goods_buy, $organics_buy, $dest['sector_id']));
                ...
                $reso = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_ore=?, ship_goods=?, ship_organics=?, ship_energy=? WHERE ship_id=?", array($playerinfo['ship_ore'], $playerinfo['ship_goods'], $playerinfo['ship_organics'], $playerinfo['ship_energy'], $playerinfo['ship_id']));
                \BlackNova\Services\Db::logDbErrors($db, $reso, __LINE__, __FILE__);
            }
            else // Dest is planet
            {
                ...
                $resp = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('planets')." SET colonists=colonists+?, fighters=fighters+?, torps=torps+? WHERE planet_id=?", array($colonists_buy, $fighters_buy, $torps_buy, $traderoute['dest_id']));
                \BlackNova\Services\Db::logDbErrors($db, $resp, __LINE__, __FILE__);

                if ($traderoute['source_type'] == 'L' || $traderoute['source_type'] == 'C')
                {
                    $resq = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_colonists=?, ship_fighters=?, torps=?, ship_energy=ship_energy+? WHERE ship_id=?", array($col_dump, $fight_dump, $torps_dump, $dist['scooped'], $playerinfo['ship_id']));
                    \BlackNova\Services\Db::logDbErrors($db, $resq, __LINE__, __FILE__);
                }
                else
                {
                    if ($setcol == 1)
                    {
                        $resr = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_colonists=?, ship_fighters=ship_fighters-?, torps=torps-?, ship_energy=ship_energy+? WHERE ship_id=?", array($col_dump, $fight_dump, $torps_dump, $dist['scooped'], $playerinfo['ship_id']));
                        \BlackNova\Services\Db::logDbErrors($db, $resr, __LINE__, __FILE__);
                    }
                    else
                    {
                        $ress = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_colonists=ship_colonists-?, ship_fighters=ship_fighters-?, torps=torps-?, ship_energy=ship_energy+? WHERE ship_id=?", array($col_dump, $fight_dump, $torps_dump, $dist['scooped'], $playerinfo['ship_id']));
                        \BlackNova\Services\Db::logDbErrors($db, $ress, __LINE__, __FILE__);
                    }
                }
            }
            if ($dist['scooped2'] != 0)
        ...
        $rest = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET turns=turns-?, credits=credits+?, turns_used=turns_used+?, sector=? WHERE ship_id=?", array($dist['triptime'], $total_profit, $dist['triptime'], $newsec, $playerinfo['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $rest, __LINE__, __FILE__);
    ...
    public static function traderouteNew($db, $pdo_db, $lang, $langvars, $bntreg, $traderoute_id, $template, $num_traderoutes, $playerinfo, $color_line1, $color_line2, $color_header)
    {
        ...
        {
            $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('traderoutes')." WHERE traderoute_id=?", array($traderoute_id));
            \BlackNova\Services\Db::logDbErrors($db, $result, __LINE__, __FILE__);
        ...
        $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('planets')." WHERE owner=? ORDER BY sector_id", array($playerinfo['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $result, __LINE__, __FILE__);
        ...
        $result = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('planets')." WHERE corp=? AND corp!=0 AND owner<>? ORDER BY sector_id", array($playerinfo['team'], $playerinfo['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $result, __LINE__, __FILE__);
    ...
    public static function traderouteCheckCompatible($db, $pdo_db, $lang, $langvars, $type1, $type2, $move, $circuit, $src, $dest, $playerinfo)
    {
        ...
        if ($move == 'warp')
        {
            $query = $db->Execute("SELECT link_id FROM ".\BlackNova\Services\Db::table('links')." WHERE link_start=? AND link_dest=?;", array($src['sector_id'], $dest['sector_id']));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            if ($query->EOF)
            {
            ...
            if ($circuit == '2')
            {
                $query = $db->Execute("SELECT link_id FROM ".\BlackNova\Services\Db::table('links')." WHERE link_start=? AND link_dest=?;", array($dest['sector_id'], $src['sector_id']));
                \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
                if ($query->EOF)
                {
    ...
    public static function traderouteDistance($db, $langvars, $type1, $type2, $start, $dest, $circuit, $playerinfo, $sells = 'N')
    {
        ...
        if ($type1 == 'L')
        {
            $query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id=?;", array($start));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            $start = $query->fields;
        }
        ...
        if ($type2 == 'L')
        {
            $query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id=?;", array($dest));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            $dest = $query->fields;
        }
    ...
    public static function traderouteCreate($db, $pdo_db, $lang, $langvars, $bntreg, $template)
    {
        ...
        if ($ptype1 == 'port')
        {
            ...
            $query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id=?;", array($port_id1));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            if (!$query || $query->EOF)
            {
        ...
        {
            $query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('planets')." WHERE planet_id=?;", array($planet_id1));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            $source = $query->fields;
            if (!$query || $query->EOF)
            {
        ...
        $pl1query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('movement_log')." WHERE sector_id=? AND ship_id = ?;", array($source['sector_id'], $playerinfo['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $pl1query, __LINE__, __FILE__);
        ...
        if ($ptype2 == 'port')
        {
            ...
            $query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id=?;", array($port_id2));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            if (!$query || $query->EOF)
            {
        ...
        {
            $query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('planets')." WHERE planet_id=?;", array($planet_id2));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            $destination = $query->fields;
            if (!$query || $query->EOF)
            {
        ...
        $pl2query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('movement_log')." WHERE sector_id=? AND ship_id = ?;", array($destination['sector_id'], $playerinfo['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $pl2query, __LINE__, __FILE__);
        ...
        if (empty ($editing))
        {
            $query = $db->Execute("INSERT INTO ".\BlackNova\Services\Db::table('traderoutes')." VALUES(NULL, ?, ?, ?, ?, ?, ?, ?);", array($src_id, $dest_id, $src_type, $dest_type, $mtype, $playerinfo['ship_id'], $circuit_type));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            echo "<p>" . $langvars['l_tdr_newtdrcreated'];
        }
        else
        {
            $query = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('traderoutes')." SET source_id=?, dest_id=?, source_type=?, dest_type=?, move_type=?, owner=?, circuit=? WHERE traderoute_id=?;", array($src_id, $dest_id, $src_type, $dest_type, $mtype, $playerinfo['ship_id'], $circuit_type, $editing));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
    ...
    {
        $query = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('traderoutes')." WHERE traderoute_id=?;", array($traderoute_id));
        \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
        ...
        {
            $query = $db->Execute("DELETE FROM ".\BlackNova\Services\Db::table('traderoutes')." WHERE traderoute_id=?;", array($traderoute_id));
            \BlackNova\Services\Db::logDbErrors($db, $query, __LINE__, __FILE__);
            $langvars['l_tdr_returnmenu'] = str_replace("[here]", "<a href='traderoute.php'>" . $langvars['l_here'] . "</a>", $langvars['l_tdr_returnmenu']);
    ...
    public static function traderouteSetsettings($db, $pdo_db, $lang, $langvars, $bntreg, $template, $playerinfo, $colonists, $fighters, $torps, $energy)
    {
        ...
        $resa = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET trade_colonists=?, trade_fighters=?, trade_torps=?, trade_energy=? WHERE ship_id=?;", array($colonists, $fighters, $torps, $energy, $playerinfo['ship_id']));
