...
\BlackNova\Services\Db::logDbErrors($db, $tr_result, __LINE__, __FILE__);
while (!$tr_result->EOF)
{
...
\BlackNova\Services\Db::logDbErrors($db, $sd_tr_result, __LINE__, __FILE__);
...
\BlackNova\Services\Db::logDbErrors($db, $ppl_tr_result, __LINE__, __FILE__);
...
\BlackNova\Services\Db::logDbErrors($db, $tmpl_tr_result, __LINE__, __FILE__);
...
else
{
    ...
    while ($i < $num_traderoutes)
    {
        ...
        else
        {
            ...
            \BlackNova\Services\Db::logDbErrors($db, $pl_result, __LINE__, __FILE__);
        ...
        else
        {
            ...
            \BlackNova\Services\Db::logDbErrors($db, $pl_dest_result, __LINE__, __FILE__);
...
if ($sectorinfo['port_type'] != "none" && mb_strlen($sectorinfo['port_type']) >0)
{
    ...
    echo "{$langvars['l_tradingport']}:&nbsp;<span style='color:#0f0;'>". ucfirst(\BlackNova\Models\Ports::from($sectorinfo['port_type'])->getLocalizedName($langvars)) ."</span>\n";
    echo "<br><br>\n";
