...
$result3 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('sector_defence')." WHERE defence_id = ?;", array($defence_id));
...
else
{
    ...
    $resulta = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." WHERE ship_id = ?;", array($defence_ship_id));
...
switch ($response)
{
    case "fight":
        ...
        {
            $countres = $db->Execute("SELECT SUM(quantity) AS totalfighters FROM ".\BlackNova\Services\Db::table('sector_defence')." WHERE sector_id = ? AND defence_type = 'F';", array($sector));
            $ttl = $countres->fields;
        ...
        else
        {
            ...
            $countres = $db->Execute("SELECT SUM(quantity) AS totalmines FROM ".\BlackNova\Services\Db::table('sector_defence')." WHERE sector_id = ? AND defence_type = 'M';", array($sector));
            $ttl = $countres->fields;
            ...
            $update4b = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_energy = ship_energy - ? WHERE ship_id = ?;", array($playerbeams, $playerinfo['ship_id']));
    ...
    case "retrieve":
        ...
        if ($quantity > 0)
        {
            $db->Execute("UPDATE ".\BlackNova\Services\Db::table('sector_defence')." SET quantity=quantity - ? WHERE defence_id = ?", array($quantity, $defence_id));
            if ($defenceinfo['defence_type'] == 'M')
            {
                $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET torps=torps + ? WHERE ship_id = ?", array($quantity, $playerinfo['ship_id']));
            }
            else
            {
                $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET ship_fighters = ship_fighters + ? WHERE ship_id = ?", array($quantity, $playerinfo['ship_id']));
            }
            $db->Execute("DELETE FROM ".\BlackNova\Services\Db::table('sector_defence')." WHERE quantity <= 0");
        }
        $stamp = date("Y-m-d H:i:s");

        $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET last_login = ?,turns = turns - 1, turns_used = turns_used + 1, sector = ? WHERE ship_id = ?;", array($stamp, $playerinfo['sector'], $playerinfo['ship_id']));
        echo "<h1>" . $title . "</h1>\n";
    ...
    case "change":
        ...
        $db->Execute("UPDATE ".\BlackNova\Services\Db::table('sector_defence')." SET fm_setting = ? WHERE defence_id = ?", array($mode, $defence_id));
        $stamp = date("Y-m-d H:i:s");
        $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET last_login = ?, turns = turns - 1, turns_used = turns_used + 1, sector = ? WHERE ship_id = ?;", array($stamp, $playerinfo['sector'], $playerinfo['ship_id']));
        if ($mode == 'attack')
        {
    ...
    default:
        ...
        {
            $result2 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." WHERE ship_id = ?;", array($defenceinfo['ship_id']));
