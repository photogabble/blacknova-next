...
if ($flag == 1)
{
    ...
    if ($ok > 0)
    {
        ...
        $move_result = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET last_login = ?,turns = turns - 1, turns_used = turns_used + 1, sector = ? WHERE ship_id = ?;", array($stamp, $sector, $playerinfo['ship_id']));
