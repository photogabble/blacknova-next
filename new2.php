...
$result = $db->Execute("SELECT email, character_name, ship_name FROM ".\BlackNova\Services\Db::table('ships')." WHERE email=? || character_name=? || ship_name=?;", array($username, $character, $shipname));
...
if ($flag == 0)
{
    ...
    $query = $db->Execute("SELECT MAX(turns_used + turns) AS mturns FROM ".\BlackNova\Services\Db::table('ships').";");
    ...
    {
        $result2 = $db->Execute("SELECT ship_id FROM ".\BlackNova\Services\Db::table('ships')." WHERE email = ?;", array($username));
        \BlackNova\Services\Db::logDbErrors($db, $result2, __LINE__, __FILE__);
        ...
        $resx = $db->Execute("INSERT INTO ".\BlackNova\Services\Db::table('zones')." VALUES (NULL, ?, ?, 'N', 'Y', 'Y', 'Y', 'Y', 'Y', 'Y', 'Y', 0);", array($character ."\'s Territory", $shipid['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $resx, __LINE__, __FILE__);

        $resx = $db->Execute("INSERT INTO ".\BlackNova\Services\Db::table('ibank_accounts')." (ship_id,balance,loan) VALUES (?,0,0);", array($shipid['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $resx, __LINE__, __FILE__);
