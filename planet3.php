...
if ($planetinfo['sells'] == 'Y')
{
    ...
    else
    {
        ...
        $trade_result = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET turns = turns - 1, turns_used = turns_used + 1, credits = credits - ?, ship_ore = ship_ore + ?, ship_organics = ship_organics + ?, ship_goods = ship_goods + ?, ship_energy = ship_energy + ? WHERE ship_id = ?;", array($total_cost, $trade_ore, $trade_organics, $trade_goods, $trade_energy, $playerinfo['ship_id']));
        \BlackNova\Services\Db::logDbErrors($db, $trade_result, __LINE__, __FILE__);

        $trade_result2 = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('planets')." SET ore = ore - ?, organics = organics - ?, goods = goods - ?, energy = energy - ?, credits = credits + ? WHERE planet_id = ?;", array($trade_ore, $trade_organics, $trade_goods, $trade_energy, $total_cost, $planet_id));
        \BlackNova\Services\Db::logDbErrors($db, $trade_result2, __LINE__, __FILE__);
        echo $langvars['l_trade_complete'] . "<br><br>";
    }
}

\BlackNova\Services\Score::updateScore($playerinfo['ship_id']);
Bnt\Text::gotoMain($db, $lang, $langvars);
Bnt\Footer::display($pdo_db, $lang, $bntreg, $template);
?>
