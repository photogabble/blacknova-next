...
$result = $db->SelectLimit("SELECT ship_id, email, recovery_time FROM ".\BlackNova\Services\Db::table('ships')." WHERE substr(MD5(password),6,8) = ?", 1, -1, array('password' => $reset_code));
...
if (!$result->EOF && $result != false)
{
    ...
    else
    {
        ...
        else
        {
            ...
            $rs = $db->Execute("UPDATE ".\BlackNova\Services\Db::table('ships')." SET password = ? WHERE ship_id = ?;", array($hashed_pass, $playerinfo['ship_id']));
            ...
            \BlackNova\Services\Db::logDbErrors($db, $recovery_update_result, __LINE__, __FILE__);
