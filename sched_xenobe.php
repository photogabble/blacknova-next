...
// Lock the tables
$resa = $db->Execute("LOCK TABLES ".\BlackNova\Services\Db::table('xenobe')." WRITE, ".\BlackNova\Services\Db::table('ships')." WRITE");
\BlackNova\Services\Db::logDbErrors($db, $resa, __LINE__, __FILE__);

/*
//\BlackNova\Services\Db::logDbErrors($db, $res, __LINE__, __FILE__);
$res = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." JOIN ".\BlackNova\Services\Db::table('xenobe')." WHERE email=xenobe_id and active='Y' and ship_destroyed='N' ORDER BY ship_id");
...
//while (!$res->EOF)
{
    ...
    if (Bnt\Rand::betterRand(1, 5) > 1)                                 // 20% Chance of not moving at all
    {
        ...
        if ($playerinfo['orders'] == 0)
        {
            ...
            $reso0 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." WHERE sector = ? AND email! = ? AND email NOT LIKE '%@xenobe' AND planet_id = 0 AND ship_id > 1", array($playerinfo['sector'], $playerinfo['email']));
            \BlackNova\Services\Db::logDbErrors($db, $res0, __LINE__, __FILE__);
            if (!$reso0->EOF)
            {
                $rowo0 = $reso0->fields;
        ...
        elseif ($playerinfo['orders'] == 1) // Orders = 1 roam
        {
            ...
            $reso1 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." WHERE sector = ? and email! = ? and ship_id > 1", array($targetlink, $playerinfo['email']));
            \BlackNova\Services\Db::logDbErrors($db, $reso1, __LINE__, __FILE__);
            if (!$reso1->EOF)
            {
                $rowo1 = $reso1->fields;
        ...
        elseif ($playerinfo['orders'] == 2)
        {
            ...
            $reso2 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." WHERE sector = ? and email! = ? and ship_id > 1", array($targetlink, $playerinfo['email']));
            \BlackNova\Services\Db::logDbErrors($db, $reso2, __LINE__, __FILE__);
            if (!$reso2->EOF)
            {
                $rowo2 = $reso2->fields;
        ...
        elseif ($playerinfo['orders'] == 3)
        {
            ...
            else
            {
                ...
                // FIND A TARGET
                // IN MY SECTOR, NOT MYSELF
                $reso3 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('ships')." WHERE sector = ? and email! = ? and ship_id > 1", array($playerinfo['sector'], $playerinfo['email']));
                \BlackNova\Services\Db::logDbErrors($db, $reso3, __LINE__, __FILE__);
                if (!$reso3->EOF)
                {
                    $rowo3 = $reso3->fields;
