...
$result4 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('universe')." WHERE sector_id = ?;", array($playerinfo['sector']));
...
$res = $db->Execute("SELECT allow_warpedit FROM ".\BlackNova\Services\Db::table('zones')." WHERE zone_id = ?;", array($sectorinfo['zone_id']));
...
$result2 = $db->Execute("SELECT * FROM ".\BlackNova\Services\Db::table('links')." WHERE link_start = ? ORDER BY link_dest ASC;", array($playerinfo['sector']));
